...
for (let i = 0; i < boxes.length; i++) {
  boxes[i].addEventListener("click", function () {
    if (boxes[i].innerHTML.trim() == "" && gameStatus == "Game On") {
      // Remplir la case avec le jeton du joueur et sa couleur
      boxes[i].style.color = playerColor;
      boxes[i].innerHTML = currentPlayer;

      // Changement de joueur
      playerColor = currentPlayer == "x" ? "green" : "red";
...
function showWinner(x, y, z) {
  boxes[x].style.background = "rgb(166, 221, 187)";
  boxes[x].style.color = "white";
  boxes[y].style.background = "rgb(166, 221, 187)";
  boxes[y].style.color = "white";
  boxes[z].style.background = "rgb(166, 221, 187)";
...
document.getElementById("reset").addEventListener("click", function () {
  for (let i = 0; i < boxes.length; i++) {
    boxes[i].innerHTML = "";
    boxes[i].style.backgroundColor = "rgb(224, 224, 224)";
